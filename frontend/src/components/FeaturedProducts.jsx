...
const FeaturedProducts = ({ featuredProducts }) => {
  ...
  useEffect(() => {
    const handleResize = () => {
      ...
      setItemsPerPage(width < 640 ? 1 : width < 1024 ? 2 : width < 1280 ? 3 : 4);
    };
    handleResize();
  ...
  const nextSlide = () => setCurrentIndex((prevIndex) => prevIndex + itemsPerPage);
  const prevSlide = () => setCurrentIndex((prevIndex) => prevIndex - itemsPerPage);
  ...
  return (
    <div className="py-12">
      <div className="container mx-auto px-4">
        <h2 className="text-3xl font-bold text-emerald-400 mb-12 text-center">FEATURED</h2>
            ...
            >
              {featuredProducts.map((product) => (
                <div key={product._id} className="w-full sm:w-1/2 lg:w-1/3 xl:w-1/4 flex-shrink-0 px-2">
                  <div className="bg-white bg-opacity-10 backdrop-blur-sm rounded-lg shadow-lg h-full transition-all duration-500 hover:shadow-xl border border-emerald-500/30 transform hover:scale-105">
                    <div className="overflow-hidden cursor-pointer" onClick={() => handleImageClick(product)}>
                      <img
                        src={product.image}
                        alt={product.name}
